

import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';
import 'package:get/get.dart';
import 'package:lottie/lottie.dart';
import 'package:material_design_icons_flutter/material_design_icons_flutter.dart';
import '../colors.dart';
import '../widgets/app_bar.dart';
import 'admin/adminHospital/admin_hospitals_private.dart';
import 'admin/adminHospital/admin_hospitals_public.dart';
import 'admin/doctor_card.dart';

class HospitalInfo extends StatefulWidget {
  @override
  State<HospitalInfo> createState() => _HospitalInfoState();
}

class _HospitalInfoState extends State<HospitalInfo> {
  String category = Get.arguments['category'].toString();
  String type = Get.arguments['type'].toString();


  @override
  Widget build(BuildContext context) {
    IconData categoryIcon;
    switch (category) {
      case 'Dental':
        categoryIcon = MdiIcons.toothOutline;
        break;
      case 'Heart':
        categoryIcon = MdiIcons.heartPlus;
        break;
      case 'Eye':
        categoryIcon = MdiIcons.eye;
        break;
      case 'Brain':
        categoryIcon = MdiIcons.brain;
        break;
      case 'Ear':
        categoryIcon = MdiIcons.earHearing;
        break;
      default:
        categoryIcon = MdiIcons.hospitalMarker; // Default icon
        break;
    }
    return Scaffold(
      body: SingleChildScrollView(
        child: Column(
          children: [
            CustomAppBar(
              title: "Hospital Information",
              backButton: true,
              signOutIcon: false,
              backgroundColor: primary,
              foregroundColor:
                  wColor,
              onBackButtonPressed: () {
                // Navigate to a specific screen
                if(type=="Private"){
                  Get.off(() => AdminHospitalsPrivate());
                }else{
                  Get.off(() => AdminHospitalsPublic());
                }

              },// Example of using a different background color
            ),
            Container(
              decoration: BoxDecoration(
                  gradient: LinearGradient(
                colors: [
                  primary.withOpacity(0.9),
                  primary.withOpacity(0.2),
                  primary.withOpacity(0),
                ],
                begin: Alignment.bottomCenter,
                end: Alignment.topCenter,
              )),
              alignment: Alignment.center,
              height: 160.0,
              child: Lottie.asset(
                "assets/Animation - hospital.json",
                width: double.infinity,

                fit: BoxFit.cover, // Adjust the fit property as needed
              ),
            ),
            SizedBox(height: 10),
            Padding(
              padding: EdgeInsets.symmetric(horizontal: 10),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 15),
                    child: Text(
                      "${Get.arguments['name'].toString()} Hospital",
                      style: TextStyle(
                        fontSize: 28,
                        fontWeight: FontWeight.bold,
                        color: primary,
                      ),
                    ),
                  ),
                  SizedBox(height: 5),
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 10),
                    child: Row(
                      children: [
                        Icon(categoryIcon, color: Colors.red, size: 30),
                        SizedBox(width: 5),
                        Text(
                          "${Get.arguments['category'].toString()} ",
                          style: TextStyle(
                            fontSize: 18,
                            color: bColor.withOpacity(0.8),
                            fontWeight: FontWeight.bold,
                          ),
                        )
                      ],
                    ),
                  ),
                  SizedBox(height: 10),

                  Container(
                    margin: EdgeInsets.all(10),
                    padding: EdgeInsets.all(15),
                    decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(15),
                      gradient: LinearGradient(
                        colors: [primary.withOpacity(0.4), primary],
                        begin: Alignment.topRight,
                        end: Alignment.bottomLeft,
                      ),
                      boxShadow: [
                        BoxShadow(
                          color: Colors.grey.withOpacity(0.5),
                          spreadRadius: 3,
                          blurRadius: 7,
                          offset: Offset(0, 3),
                        ),
                      ],
                    ),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Row(
                          children: [
                            Icon(
                              Icons.public,
                              color: Colors.white,
                            ),
                            SizedBox(width: 10),
                            Expanded(
                              child: Text(
                                "${Get.arguments['type'].toString()}",
                                style: TextStyle(
                                  color: Colors.white,
                                  fontSize: 16,
                                  fontWeight: FontWeight.bold,
                                ),
                              ),
                            ),
                          ],
                        ),
                        SizedBox(height: 10),
                        Row(
                          children: [
                            Icon(
                              Icons.location_on,
                              color: Colors.white,
                            ),
                            SizedBox(width: 10),
                            Expanded(
                              child: Text(
                                "${Get.arguments['location'].toString()}",
                                style: TextStyle(
                                  color: Colors.white,
                                  fontSize: 16,
                                  fontWeight: FontWeight.bold,
                                ),
                              ),
                            ),
                          ],
                        ),
                        SizedBox(height: 10),
                        Row(
                          children: [
                            Icon(
                              Icons.phone,
                              color: Colors.white,
                            ),
                            SizedBox(width: 10),
                            Text(
                              "${Get.arguments['contact'].toString()}",
                              style: TextStyle(
                                color: Colors.white,
                                fontSize: 16,
                                fontWeight: FontWeight.bold,
                              ),
                            ),
                          ],
                        ),
                      ],
                    ),
                  ),
                ],
              ),
            ),
            SizedBox(height: 8),
            Text("Hospital Doctors",
                style: TextStyle(
                  fontSize: 20,
                  // backgroundColor: pColor.withOpacity(0.8),
                  color: bColor.withOpacity(0.8),
                  fontWeight: FontWeight.bold,
                )),
            DoctorsList(hospitalName: "${Get.arguments['name'].toString()}",),
          ],
        ),
      ),
    );
  }
}
